...
/// @title DefiPair
/// @notice Constant-product AMM pair (x*y=k) with 0.3% swap fee.
///         LP tokens represent proportional share of the pool.
contract DefiPair is ERC20, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public constant MINIMUM_LIQUIDITY = 1000;
    uint32  public constant FEE_NUMERATOR    = 997;   // 0.3% fee → multiply by 997/1000
    uint32  public constant FEE_DENOMINATOR  = 1000;
    ...
    // Protocol fee (accrued in LP tokens if feeTo != address(0))
    ...
    constructor() ERC20("DeFi LP", "DEFI-LP") {
        factory = msg.sender;
    }

    /// @notice Called once by factory at pair creation
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IDefiFactory(factory).feeTo();
    ...
    /// @notice Add liquidity — caller must have sent tokens to this contract first
    ...
    /// @notice Execute swap — tokens must be sent to this contract first
...
interface IDefiFactory {
    function feeTo() external view returns (address);
}

/// @notice Fixed-point Q112.112 library for price accumulators
library UQ112x112 {
